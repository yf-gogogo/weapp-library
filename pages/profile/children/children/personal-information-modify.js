...
Page({
  ...
  onSubmit: function() {
    ...
    Promise.all([
      this.data.idCardFront.indexOf("mymoonlight") == -1 ? userApi.uploadIdCardImg(this.data.idCardFront) : this.data.idCardFront,
      this.data.idCardBack.indexOf("mymoonlight") == -1 ? userApi.uploadIdCardImg(this.data.idCardBack) : this.data.idCardBack,
    ]).then((res) => {
      ...
      return userApi.updateUserInfoByPhone(getApp().globalData.phone, params)
    }).then(res => {
      wx.showToast({
        title: "成功",
        duration: 1500,
